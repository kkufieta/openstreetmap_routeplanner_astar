...
TEST_F(RoutePlannerTest, TestAddNeighbors) {
  ...
  std::vector<float> start_neighbor_g_vals{0.10671431, 0.082997195, 0.051776856,
                                           0.055291083};
  std::vector<float> start_neighbor_h_vals{1.1828455, 1.0998145, 1.0858033,
                                           1.1831238};
  auto neighbors = start_node->neighbors;
  EXPECT_EQ(neighbors.size(), 4);

  // Sort neighbor values (bubble sort based on g value)
  for (int i = 0; i < start_neighbor_g_vals.size(); i++) {
    bool sorted = true;
    for (int j = 0; j < start_neighbor_g_vals.size() - 1; j++) {
      if (start_neighbor_g_vals[j] > start_neighbor_g_vals[j + 1]) {
        sorted = false;
        float temp_g = start_neighbor_g_vals[j + 1];
        float temp_h = start_neighbor_h_vals[j + 1];
        start_neighbor_g_vals[j + 1] = start_neighbor_g_vals[j];
        start_neighbor_h_vals[j + 1] = start_neighbor_h_vals[j];
        start_neighbor_g_vals[j] = temp_g;
        start_neighbor_h_vals[j] = temp_h;
      }
    }
    if (sorted) {
      break;
  ...
  // Sort neighbors (bubble sort based on g value)
  for (int i = 0; i < neighbors.size(); i++) {
    bool sorted = true;
    for (int j = 0; j < neighbors.size() - 1; j++) {
      if (neighbors[j]->g_value > neighbors[j + 1]->g_value) {
        sorted = false;
        float temp_g = neighbors[j + 1]->g_value;
        float temp_h = neighbors[j + 1]->h_value;
        neighbors[j + 1]->g_value = neighbors[j]->g_value;
        neighbors[j + 1]->h_value = neighbors[j]->h_value;
        neighbors[j]->g_value = temp_g;
        neighbors[j]->h_value = temp_h;
      }
    }
    if (sorted) {
      break;
